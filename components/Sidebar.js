...
const Sidebar = () => {
  ...
  return (
    <div className="hidden sm:flex flex-col items-center xl:items-start xl:w-[340px] p-2 fixed h-full border-r border-gray-400 pr-0 xl:pr-8">
      ...
      <div
        className="text-[#d9d9d9] flex items-center justify-center mt-auto hoverEffect xl:ml-auto xl:-mr-5 px-4 py-y"
        onClick={signOut}
      >
        <Image
        ...
        <div className="hidden xl:inline leading-5">
          <h4 className="font-bold ml-2">{session?.user?.name}</h4>
          <h4 className="text-[#6e767d] ml-2">@{session?.user?.tag}</h4>
        </div>
        <BsThreeDots className="h-5 xl:inline ml-10" />
      </div>
    </div>
  );
};
